...
const ANSWER_FIELDS = [
  ...
  'previously_engaged','widow_duration','last_divorce_date','divorce_count',
  ...
  'governorate','area','area_type','marital_home_location','housing_type','family_house_living',
...
app.get('/api/profiles', async (req, res) => {
  try {
    const {
      gender, minAge, maxAge, governorate, maritalStatus, wantsChildren,
      education, job, religiosityLevel, smoker, page = 1, limit = 12
    } = req.query;
    ...
    if (governorate) { where.push('p.governorate LIKE ?'); params.push(`%${governorate}%`); }
    if (maritalStatus) { where.push('p.marital_status = ?'); params.push(maritalStatus); }
    if (wantsChildren) { where.push('p.wants_more_children = ?'); params.push(wantsChildren); }
    if (education) { where.push('p.education LIKE ?'); params.push(`%${education}%`); }
    ...
    const [rows] = await pool.query(
      `SELECT u.id, u.gender, p.age, p.weight, p.height, p.skin_color, p.education, p.job,
              p.marital_status, p.governorate, p.religiosity_level, p.smoker, p.about_me,
              p.wants_more_children
...
app.post('/api/admin/seed-test-data', requireAdmin, async (req, res) => {
  try {
    ...
    const educations = ['ثانوية عامة','دبلوم','بكالوريوس تجارة','بكالوريوس هندسة','بكالوريوس طب','ماجستير','دكتوراه'];
    ...
    for (let i = 0; i < count; i++) {
      ...
      const phone = `01${randInt(0,2)}${String(randInt(10000000, 99999999)).padStart(8,'0')}${i}`;
      ...
      const [userResult] = await pool.query(
        ...
        [phone.slice(0,20), password_hash, gender]
      );
      ...
      await pool.query(
        `INSERT INTO profile_answers (user_id, ${cols.join(',')}) VALUES (?, ${placeholders})`,
        [uid, ...values]
      );
    }
