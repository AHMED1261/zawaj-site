...
app.post('/api/admin/seed-test-data', requireAdmin, async (req, res) => {
  try {
    // طبقة حماية إضافية: نتأكد من وجود الأعمدة لحظة الاستخدام مباشرة،
    // مش هنعتمد بس على نجاح مايجريشن وقت بدء تشغيل السيرفر
    ...
    for (const [col, def] of requiredColumns) {
      const [rows] = await pool.query(
        `SELECT COUNT(*) as cnt FROM information_schema.columns
         WHERE table_schema = DATABASE() AND table_name = 'profile_answers' AND column_name = ?`,
        [col]
      );
      if (rows[0].cnt === 0) {
        await pool.query(`ALTER TABLE profile_answers ADD COLUMN ${col} ${def}`);
      }
    }

    const count = Math.min(Number(req.body.count) || 200, 500);

    // كل مرة تولّد بيانات جديدة، نمسح الدفعة التجريبية القديمة الأول
    // عشان الزرار يشتغل من غير أخطاء تكرار حتى لو اتضغط أكتر من مرة
    await pool.query("DELETE FROM users WHERE phone LIKE '0000%'");
    ...
    try {
      ...
      firstId = userResult.insertId; // أول id في الدفعة - باقي المعرفات متتالية بعده
