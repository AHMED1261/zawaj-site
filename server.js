...
async function adminAuditLog(adminId, action, targetUserId = null, details = {}) {
  try {
    await pool.query(
...
async function ensureRecentColumns() {
  ...
  const checks = [
    ...
    ['profile_answers', 'children_details', 'TEXT'],
    ['profile_answers', 'past_marriage_details', 'TEXT'],
    ['profile_answers', 'family_background', 'TEXT']
  ];
...
const ANSWER_FIELDS = [
  ...
  'marital_status','has_children','children_count','children_ages','custody','children_details',
  'widow_duration','last_divorce_date','divorce_count','past_marriage_details',
  'current_wives_count','wants_polygamy','polygamy_with_first_wife_knowledge','wants_more_children',
  'father_job','mother_job','siblings_count','siblings_education','family_background',
  'governorate','area','area_type','marital_home_location','marital_home_area','housing_type','family_house_living',
  'prays_regularly','smoker','quran_memorization','watches_series',
  ...
  'accepted_governorates','partner_education_preference','partner_work_preference',
  'wants_publish_social','contacted_before','notes'
,'dob','hijab_type','accepted_hijab_types','personal_photos','id_front','id_back','admin_phone','admin_whatsapp','education_specialization'];
...
app.post('/api/register', authLimiter, csrfProtection, async (req, res, next) => {
  try {
    ...
    if (!phone || !password) {
      return res.status(400).json({ error: 'من فضلك املأ رقم الهاتف وكلمة المرور' });
    }
    // Password Policy
    if (password.length < 6) {
      return res.status(400).json({ error: 'كلمة المرور ضعيفة. يجب أن تحتوي على الأقل 6 أحرف.' });
    }
...
// ---------- Google Login ----------
app.post('/api/auth/google', authLimiter, csrfProtection, async (req, res, next) => {
  try {
    const { token } = req.body;
    if (!token) return res.status(400).json({ error: 'لم يتم إرسال توكن جوجل' });
    ...
    try {
      ...
      return res.status(401).json({ error: 'توكن جوجل غير صالح' });
    }
    ...
    const [rows] = await pool.query('SELECT * FROM users WHERE google_id = ? OR email = ?', [googleId, email]);
    ...
    if (rows.length > 0) {
      ...
      securityLog('Successful_Google_Login', { userId: user.id, role: 'user' });
      const sessionToken = signToken({ id: user.id, phone: user.phone, role: 'user' });
      res.cookie('token', sessionToken, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 30*24*60*60*1000 });
      return res.json({ token: sessionToken, status: user.status, gender: user.gender });
    } else {
      const [result] = await pool.query(
      ...
      securityLog('User_Google_Registered', { userId: result.insertId, role: 'user' });
      const sessionToken = signToken({ id: result.insertId, phone: null, role: 'user' });
      res.cookie('token', sessionToken, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 30*24*60*60*1000 });
      return res.status(201).json({ token: sessionToken, message: 'تم إنشاء الحساب بنجاح عبر جوجل' });
    }
  } catch (err) {
    next(err);
  }
...
app.post('/api/profile/submit', requireAuth, upload.fields([{name:'personal_photos', maxCount:10}, {name:'id_front', maxCount:1}, {name:'id_back', maxCount:1}]), csrfProtection, async (req, res, next) => {
  try {
    let data = sanitizeData(req.body);

    if (req.files) {
      if (req.files.personal_photos) {
        data.personal_photos = JSON.stringify(req.files.personal_photos.map(f => f.filename));
      }
      if (req.files.id_front) {
        data.id_front = req.files.id_front[0].filename;
      }
      if (req.files.id_back) {
        data.id_back = req.files.id_back[0].filename;
      }
    }

    if (Array.isArray(data.accepted_hijab_types)) {
      data.accepted_hijab_types = data.accepted_hijab_types.join('، ');
    }
    ...
    if (data.gender) {
      await pool.query('UPDATE users SET gender = ? WHERE id = ?', [data.gender, req.user.id]);
    }
    ...
    const conditionallyOptional = new Set([
      'income_level', 'children_details', 'past_marriage_details', 'family_background', 'partner_skin_color_preference', 'notes',
      'children_count','children_ages','custody','widow_duration','last_divorce_date','divorce_count',
      'father_job','mother_job','siblings_count','siblings_education','religious_scholars_followed',
      'current_wives_count','wants_polygamy','polygamy_with_first_wife_knowledge','relative_relation',
      'family_house_living','filled_with_knowledge','has_beard','accepts_with_children','accepts_children_with_father_custody'
    ,'marital_home_location','marital_home_area','hijab_type','accepted_hijab_types','personal_photos','id_front','id_back','dob','admin_whatsapp','education_specialization']);

    if (data.gender === 'أنثى') {
      conditionallyOptional.add('has_beard');
      conditionallyOptional.add('accepted_hijab_types');
      conditionallyOptional.add('marital_home_location');
      conditionallyOptional.add('marital_home_area');
    }
    if (data.gender === 'ذكر') {
      conditionallyOptional.add('hijab_type');
    }
    if (data.marital_status === 'أعزب' || data.marital_status === 'كتب كتاب') {
      conditionallyOptional.add('has_children');
      conditionallyOptional.add('current_wives_count');
      conditionallyOptional.add('wants_polygamy');
      conditionallyOptional.add('polygamy_with_first_wife_knowledge');
    }
    ...
    if (data.housing_type !== 'سكن عائلة') {
      conditionallyOptional.add('family_house_living');
    }
    if (data.form_filled_by !== 'قريب') {
      conditionallyOptional.add('relative_relation');
      conditionallyOptional.add('filled_with_knowledge');
    }
    if (data.has_children !== 'yes') {
      conditionallyOptional.add('children_count');
      conditionallyOptional.add('children_ages');
      conditionallyOptional.add('custody');
      conditionallyOptional.add('children_details');
    }
    if (!data.partner_marital_status_accepted || (!data.partner_marital_status_accepted.includes('مطلق/ة') && !data.partner_marital_status_accepted.includes('أرمل/ة'))) {
      conditionallyOptional.add('accepts_with_children');
      conditionallyOptional.add('accepts_children_with_father_custody');
    }
    ...
    if (userStatus === 'approved' || userStatus === 'needs_revision') {
      ...
      const values = columns.map(f => { if (data[f] === '') return null; if (Array.isArray(data[f])) return JSON.stringify(data[f]); return data[f]; });
...
app.post('/api/profile/draft', requireAuth, upload.fields([{name:'personal_photos', maxCount:10}, {name:'id_front', maxCount:1}, {name:'id_back', maxCount:1}]), csrfProtection, async (req, res, next) => {
  try {
    let data = sanitizeData(req.body);
...
app.get('/api/user/profile', requireAuth, async (req, res, next) => {
  try {
    ...
    // Also fetch gender from the users table (stored separately from profile_answers)
    const [userRows] = await pool.query('SELECT gender FROM users WHERE id = ?', [req.user.id]);
    const gender = userRows.length ? userRows[0].gender : null;

    if (!rows.length) {
      // Return at minimum the gender so the form can prefill it
      return res.json(gender ? { gender } : {});
    }
    // Merge gender into the profile data
    res.json({ ...rows[0], gender });
  } catch (err) {
    next(err);
  }
});
...
app.get('/api/pages/:slug', async (req, res, next) => {
  try {
    const [rows] = await pool.query('SELECT title, content FROM pages WHERE slug = ?', [req.params.slug]);
    if (!rows.length) return res.status(404).json({ error: 'الصفحة غير موجودة' });
    res.json(rows[0]);
  } catch (err) { next(err); }
});
...
app.get('/api/profiles', async (req, res, next) => {
  try {
    ...
    const {
      gender, minAge, maxAge, governorate, maritalHomeLocation, maritalStatus, wantsChildren, hasChildren,
    ...
    if (hasChildren) { where.push('p.has_children = ?'); params.push(hasChildren); }
    ...
    const housingTypes = toArray(housingType);
    if (housingTypes.length) { where.push(`p.housing_type IN (${housingTypes.map(() => '?').join(',')})`); params.push(...housingTypes); }
    ...
    if (hijab_type) {
      const hijabs = hijab_type.split(',');
      const qs = hijabs.map(() => '?').join(',');
      where.push(`p.hijab_type IN (${qs})`);
      params.push(...hijabs);
    }
...
app.get('/api/profiles/:id', async (req, res, next) => {
  try {
    ...
    const displayFields = ANSWER_FIELDS.filter(f => !['wants_publish_social', 'contacted_before', 'notes', 'dob', 'id_front', 'id_back', 'personal_photos', 'admin_phone', 'admin_whatsapp'].includes(f));
...
app.post('/api/admin/login', authLimiter, csrfProtection, async (req, res, next) => {
  try {
    ...
    const token = signToken({ id: rows[0].id, username, role: 'admin', admin_role: rows[0].role }, '2d');
...
app.get('/api/admin/users', requireAdmin, async (req, res, next) => {
  try {
    ...
    let baseWhere = `u.status = ?`;
    let queryParams = [status];
    
    if (search) {
      baseWhere += ` AND u.phone LIKE ?`;
    ...
    if (status === 'pending_edits') {
      [rows] = await pool.query(
        `SELECT u.id, u.phone, u.gender, u.status, u.created_at, p.*, e.data as pending_data
         ...
         WHERE e.status = 'pending' ${search ? 'AND u.phone LIKE ?' : ''} ORDER BY e.created_at ASC`,
        search ? [`%${search}%`] : []
      );
    } else {
      [rows] = await pool.query(
        `SELECT u.id, u.phone, u.gender, u.status, u.created_at, p.*
         FROM users u LEFT JOIN profile_answers p ON p.user_id = u.id
         WHERE ${baseWhere} ORDER BY u.created_at ASC`,
        queryParams
      );
    }
    res.json(rows);
...
app.get('/api/admin/audit-logs', requireSuperAdmin, async (req, res, next) => {
  try {
    const [rows] = await pool.query(`
      SELECT l.*, a.username 
...
app.post('/api/admin/users/:id/approve', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Profile_Approved', userId);
...
app.post('/api/admin/users/:id/reject', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Profile_Rejected', req.params.id, { reason });
...
app.post('/api/admin/users/:id/suspend', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Account_Suspended', req.params.id);
...
app.post('/api/admin/users/:id/revise', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Profile_Needs_Revision', req.params.id, { reason });
...
app.post('/api/admin/users/:id/approve-edit', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Profile_Edit_Approved', req.params.id);
...
app.post('/api/admin/users/:id/reject-edit', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Profile_Edit_Rejected', req.params.id);
...
app.post('/api/admin/requests/:request_id/approve', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Marriage_Request_Approved', null, { requestId: req.params.request_id });
...
app.post('/api/admin/requests/:request_id/reject', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Marriage_Request_Rejected', null, { requestId: req.params.request_id });
...
app.patch('/api/admin/users/:id/profile', requireAdmin, csrfProtection, async (req, res, next) => {
  try {
    ...
    await adminAuditLog(req.admin.id, 'Admin_Updated_Profile', req.params.id, { updatedFields: allowedFields });
...
// حذف حساب نهائيًا - المشرف العام بس
app.delete('/api/admin/users/:id', requireSuperAdmin, csrfProtection, async (req, res, next) => {
  try {
    await pool.query('DELETE FROM users WHERE id = ?', [req.params.id]);
    securityLog('Admin_Deleted_User', { adminId: req.admin.id, targetUserId: req.params.id });
    await adminAuditLog(req.admin.id, 'Admin_Deleted_User', req.params.id);
...
// توليد بيانات تجريبية (200 بروفايل معتمد) لاختبار الفلاتر - محمي بحساب المشرف العام
// ملحوظة أداء: بيستخدم إدخال مجمّع (Bulk Insert) بدل استعلام منفصل لكل بروفايل
// عشان يتجنب أي timeout من السيرفر مع أعداد كبيرة
app.post('/api/admin/seed-test-data', requireSuperAdmin, csrfProtection, async (req, res, next) => {
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      profileDataList.push({
        ...
        skin_color: rand(skinColors), has_beard: gender === 'ذكر' ? rand(yesNo) : null,
        ...
        wants_polygamy: gender === 'ذكر' ? rand(yesNo) : null,
    ...
    await adminAuditLog(req.admin.id, 'Seed_Test_Data', null, { count });
...
// حذف كل الحسابات التجريبية دفعة واحدة - محمي بحساب المشرف العام
app.delete('/api/admin/test-data', requireSuperAdmin, csrfProtection, async (req, res, next) => {
  if (process.env.NODE_ENV === 'production') {
  ...
  try {
    ...
    await adminAuditLog(req.admin.id, 'Delete_Test_Data', null, { deletedCount: result.affectedRows });
