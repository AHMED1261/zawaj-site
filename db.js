...
async function initSchema() {
  ...
  try {
    ...
    await ensureColumn('profile_answers', 'children_details', 'TEXT');
    await ensureColumn('profile_answers', 'past_marriage_details', 'TEXT');
    await ensureColumn('profile_answers', 'family_background', 'TEXT');
    await ensureColumn('profile_answers', 'admin_phone', 'VARCHAR(50)');
    await ensureColumn('profile_answers', 'admin_whatsapp', 'VARCHAR(50)');
    await ensureColumn('profile_answers', 'education_specialization', 'VARCHAR(255)');
    await ensureColumn('profile_answers', 'admin_phone', 'VARCHAR(50)');
    await ensureColumn('profile_answers', 'admin_whatsapp', 'VARCHAR(50)');
    await ensureColumn('profile_answers', 'dob', 'DATE');
    await ensureColumn('profile_answers', 'hijab_type', "ENUM('hijab','khimar','niqab')");
    await ensureColumn('profile_answers', 'accepted_hijab_types', 'VARCHAR(255)');
    await ensureColumn('profile_answers', 'personal_photos', 'TEXT');
    await ensureColumn('profile_answers', 'id_front', 'VARCHAR(255)');
    await ensureColumn('profile_answers', 'id_back', 'VARCHAR(255)');
    await ensureColumn('users', 'email', 'VARCHAR(255) UNIQUE NULL');
    await ensureColumn('users', 'google_id', 'VARCHAR(255) UNIQUE NULL');
    ...
    const migrations = [
      ...
      `ALTER TABLE profile_answers MODIFY COLUMN marital_status ENUM('single','married','divorced','widowed','separated','katb_ketab')`,
      `ALTER TABLE profile_answers MODIFY COLUMN has_beard ENUM('yes','no','no_beard','short','medium','long')`,
      `ALTER TABLE profile_answers MODIFY COLUMN prays_regularly ENUM('yes','no','all','most','sometimes','none')`,
      `ALTER TABLE users MODIFY COLUMN phone VARCHAR(20) UNIQUE NULL`,
      `ALTER TABLE users MODIFY COLUMN password_hash VARCHAR(255) NULL`,
      `ALTER TABLE users MODIFY COLUMN gender ENUM('male','female') NULL`,
      // Fix partner_education_preference: was ENUM, now stores Arabic text from checkboxes
      `ALTER TABLE profile_answers MODIFY COLUMN partner_education_preference TEXT`,
      // Fix partner_work_preference: was ENUM, keep backward compatible but expand
      `ALTER TABLE profile_answers MODIFY COLUMN partner_work_preference VARCHAR(255)`,
      // Fix accepted_governorates: TEXT is fine
      // Fix form_filled_by: add 'myself' as synonym of 'self'
      `ALTER TABLE profile_answers MODIFY COLUMN form_filled_by ENUM('self','relative','myself')`,
      // Fix area_type to include city/village
      `ALTER TABLE profile_answers MODIFY COLUMN area_type ENUM('popular','medium','upscale','city','village')`,
      // Ensure marital_home_location is long enough
      `ALTER TABLE profile_answers MODIFY COLUMN marital_home_location VARCHAR(255)`
    ];
    ...
    await ensureColumn('admins', 'role', "ENUM('superadmin', 'moderator') DEFAULT 'superadmin'");

    if (process.env.ADMIN_USERNAME && process.env.ADMIN_PASSWORD) {
      const [existing] = await conn.query('SELECT id FROM admins WHERE username = ?', [process.env.ADMIN_USERNAME]);
      if (!existing.length) {
        const hash = await bcrypt.hash(process.env.ADMIN_PASSWORD, 10);
        await conn.query('INSERT INTO admins (username, password_hash, role) VALUES (?, ?, "superadmin")', [process.env.ADMIN_USERNAME, hash]);
        console.log(`تم إنشاء حساب المشرف: ${process.env.ADMIN_USERNAME}`);
      }
    }

    // جدول سجل نشاطات المشرفين (Audit Logs)
    await conn.query(`
    ...
    // إدراج الصفحات الأساسية إذا لم تكن موجودة
    const defaultPages = [
      ['about', 'عن الموقع', '<p>نحن موقع زواج إسلامي يهدف للتوفيق بين الراغبين في الزواج الشرعي.</p>'],
      ['privacy', 'سياسة الخصوصية', '<p>نحن نحترم خصوصيتك ونحافظ على سرية بياناتك.</p>'],
      ['terms', 'الشروط والأحكام', '<p>استخدامك للموقع يعني موافقتك على شروطنا وأحكامنا.</p>'],
      ['contact', 'اتصل بنا', '<p>يمكنك التواصل معنا عبر البريد الإلكتروني أو الهاتف.</p>'],
      ['faq', 'الأسئلة الشائعة', '<p>هنا تجد إجابات على الأسئلة الشائعة.</p>']
    ];
